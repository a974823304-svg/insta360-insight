  ...
  <div class="page" v-loading="store.loading">
    <header class="page-head">
      <div class="title-block">
        <h1>品牌分析 <span class="info" title="数据每 5 分钟刷新一次">ⓘ</span></h1>
        <p class="sub">{{ dateRangeText }} · Insta360 品牌声量与合作数据洞察</p>
      </div>
    </header>
    ...
    <section class="grid-row-wide">
      <div class="card trend-card">
        <div class="card-head"><span class="card-title">品牌声量趋势</span></div>
    ...
    <section class="grid-row-3">
      <div class="card donut-card">
        <div class="card-head"><span class="card-title">平台声量分布</span></div>
        <div class="card-body donut-body">
          <div class="donut-chart"><PlatformDonut :data="store.platforms" centerLabel="声量" /></div>
          <ul class="legend-col">
      ...
      <div class="card donut-card">
        <div class="card-head"><span class="card-title">品牌情感分布</span></div>
        <div class="card-body donut-body">
          <div class="donut-chart"><AgeDonut :data="store.sentiment" centerLabel="情感占比" center-value="100%" /></div>
          <ul class="legend-col">
            <li v-for="s in store.sentiment" :key="s.bucket">
              <span class="dot" :style="{ background: s.color }" />
              <span class="lab">{{ s.bucket }}</span>
              <span class="pct num">{{ s.share }}%</span>
            </li>
          </ul>
        </div>
      </div>
      <div class="card kw-card">
        <div class="card-head"><span class="card-title">品牌热词</span></div>
        <div class="card-body kw-body">
          <ul class="kw-list">
            <li v-for="k in store.keywords" :key="k.word" class="kw-item">
              <span class="kw-word">{{ k.word }}</span>
              <span class="kw-bar"><i :style="{ width: (k.weight / maxKw * 100) + '%' }" /></span>
              <span class="kw-val num">{{ k.weight }}</span>
            </li>
          </ul>
        </div>
      </div>
    ...
    <section class="grid-row-full">
      <div class="card list-card">
        <div class="card-head"><span class="card-title">合作品牌总表</span></div>
        <div class="card-body"><DataTable :columns="brandColumns" :rows="store.list" searchable row-key="name" /></div>
      </div>
    </section>
  </div>
</template>
...
const maxKw = computed(() => Math.max(1, ...store.keywords.map(k => k.weight)))
...
const brandColumns = [
  { key: 'name', label: '合作品牌', sortable: false },
  { key: 'industry', label: '行业', sortable: false },
  { key: 'contents', label: '合作内容', sortable: true },
  { key: 'exposure', label: '曝光', sortable: true, formatter: (r, c, v) => fmtInt(v) },
  { key: 'engagement', label: '互动', sortable: true, formatter: (r, c, v) => fmtInt(v) },
...
.kw-body { height: 180px; display: flex; flex-direction: column; justify-content: center; gap: 8px; padding: 4px 2px; }
.kw-list { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 8px; }
.kw-item { display: grid; grid-template-columns: minmax(0, 72px) minmax(0, 1fr) 32px; align-items: center; gap: 8px; font-size: 11px; color: var(--text-secondary); }
.kw-word { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.kw-bar { height: 8px; background: rgba(255, 255, 255, 0.06); border-radius: 999px; overflow: hidden; }
.kw-bar i { display: block; height: 100%; border-radius: 999px; background: linear-gradient(90deg, #3DD9EB, #5EA1FF); }
.kw-val { text-align: right; color: var(--text-primary); font-weight: 600; }
